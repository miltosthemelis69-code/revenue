  ...
  return (
    <>
      <div style={{ marginBottom: 18 }}>
        <h2 style={{ fontSize: 18, fontWeight: 600, margin: "0 0 6px" }}>Public dashboard</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 14 }}>
          ...
          <div className="card" style={{ padding: "18px 20px" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              ...
              <button type="button" className="ghost" onClick={copyLink} disabled={!enabled} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                {copied ? <Check size={14} color={theme.green} /> : <Copy size={14} />}
                {copied ? "Copied" : "Copy"}
              </button>
            </div>
        ...
        {/* Public preview mock */}
        <div>
          <div style={{ fontSize: 11, color: theme.dim, marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <div style={{ width: 18, height: 18, borderRadius: 4, background: `linear-gradient(135deg,${theme.accent},${theme.accentDark})` }} />
              <span style={{ fontWeight: 600, fontSize: 14 }}>statly.app</span>
              <span style={{ fontSize: 11, color: theme.dim, marginLeft: "auto" }}>Powered by Ledger</span>
            </div>

            {showLive && (
              <div style={{ fontSize: 12, color: theme.green, marginBottom: 12 }}>
            ...
            {showRevenue && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 16 }}>
                {KPIS.filter((k) => ["Revenue (30d)", "Visitors (30d)", "Conversion rate"].includes(k.label)).map((k) => (
                  <div key={k.label} style={{ background: theme.card, borderRadius: 8, padding: "12px 14px", border: `1px solid ${theme.rowBorder}` }}>
