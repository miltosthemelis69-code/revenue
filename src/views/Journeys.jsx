  ...
  return (
    <>
      <div style={{ marginBottom: 18 }}>
        <h2 className="serif" style={{ fontSize: 20, fontWeight: 400, margin: "0 0 6px" }}>Visitor journeys</h2>
      ...
      {VISITOR_JOURNEYS.map((j) => (
        <div key={j.sessionId} className="panel" style={{ padding: "18px 20px", marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <Route size={15} color={theme.muted} />
              <span className="mono" style={{ fontSize: 13 }}>{j.visitorId}</span>
              <span style={{ fontSize: 12, color: theme.faint }}>· session {j.sessionId}</span>
              <span style={{ fontSize: 12, color: theme.faint }}>· {j.duration}</span>
            </div>
            {j.converted ? (
              <span style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: theme.credit }}>
                <CheckCircle2 size={13} /> Converted · {money(j.revenue)}
              </span>
            ) : (
              <span style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: theme.faint }}>
          ...
          <div style={{ position: "relative", paddingLeft: 18 }}>
            ...
            {j.pages.map((p, idx) => (
              <div key={idx} style={{ position: "relative", paddingBottom: idx === j.pages.length - 1 ? 0 : 18 }}>
                <div style={{ position: "absolute", left: -18, top: 3, width: 8, height: 8, borderRadius: "50%", background: theme.credit, border: `2px solid ${theme.bg}` }} />
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  <span className="mono" style={{ fontSize: 13.5 }}>{p.path}</span>
                  <span style={{ fontSize: 11.5, color: theme.faint }}>{p.time} · {p.duration}</span>
                </div>
                {p.referrer && (
                ...
                {j.events.filter((e) => e.time.split(":")[0] === p.time.split(":")[0]).map((e) => (
                  <div key={e.name} style={{ fontSize: 11.5, color: theme.credit, marginTop: 4 }} className="mono">
                    ↳ {e.name}
                  </div>
                ))}
              </div>
