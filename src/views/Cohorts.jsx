  ...
  return (
    <>
      <div style={{ marginBottom: 18 }}>
        <h2 className="serif" style={{ fontSize: 20, fontWeight: 400, margin: "0 0 6px" }}>Cohorts & channel LTV</h2>
      ...
      <div className="panel" style={{ padding: "18px 20px" }}>
        <div style={cardTitleStyle()}>Retained revenue by acquisition channel</div>

        <div style={{ display: "grid", gridTemplateColumns: "110px repeat(6, 1fr)", gap: 4, fontSize: 10.5, color: theme.faint, padding: "0 4px 10px" }}>
          <span>Channel</span>
        ...
        {COHORT_ANALYSIS.map((cohort, ridx) => (
          <div key={cohort.cohort} style={{ marginBottom: 18 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 11.5, color: theme.faint }}>{cohort.cohortSize.toLocaleString()} customers in cohort</span>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "110px repeat(6, 1fr)", gap: 4, alignItems: "center", marginTop: 3 }}>
              <span></span>
              {cohort.months.map((m) => (
                <div key={m.month} className="mono" style={{ textAlign: "center", fontSize: 10.5, color: theme.faint }}>
                  {money(m.revenue)}
                </div>
              ))}
            </div>
        ...
        <div style={{ marginTop: 6, padding: "12px 14px", background: theme.panel, borderRadius: 3, fontSize: 12, color: theme.muted, lineHeight: 1.5 }}>
